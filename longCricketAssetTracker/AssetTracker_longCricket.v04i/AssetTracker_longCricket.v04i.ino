/* Cricket Asset Tracker contains:
   ...
   BMA400 accelerometer, BME280 pressure/humidity/temperature sensors
   ...
   4.2 MHz clock speed; this reduction means no serial
   through the USB.  
...
#define VbatMon A1   // ADC for LiPo battery voltage
#define userButton 2 // user button
...
//BMA280 definitions
#define BMA400_intPin1 3    // interrupt1 pin definitions, no motion detect
#define BMA400_intPin2 A4   // interrupt2 pin definitions, wakeup
...
void setup()
{
  ...
  pinMode(BMA400_intPin1, INPUT);  // define BMA400 sleep interrupt pin as L082 inputs
  pinMode(BMA400_intPin2, INPUT);  // define BMA400  wake interrupt pin as L082 inputs
  ...
  pinMode(GNSS_backup, OUTPUT);   // power for CAM M8Q RTC backup
  ...
  BME280.I2Cscan(); // should detect BME280 at 0x77 and BMA400 at 0x14 
  delay(1000);

  GNSS.begin(Serial1, GNSS.MODE_UBLOX, GNSS.RATE_1HZ); // Start GNSS, LDO enable pin D5
  ...
  VBAT = 1.27f * VDDA * ( (float) analogRead(VbatMon) ) / 4096.0f;
  digitalWrite(VbatCtl, LOW); // enable battery voltage monitor control pin
  ...
  if(c == 0x90 && d == 0x60) // check if all I2C sensors with WHO_AM_I have acknowledged
  {
  Serial.println("BMA400 + BME280 are online..."); Serial.println(" ");
...
// for testing
    ...
    NoMotionActivityTimer.start(callbackNoMotionActivity, 0, 360000);    // low  freq (two hours) timer
    InMotionActivityTimer.start(callbackInMotionActivity, 100000,   360000);    // high freq (one minute) timer
...
void loop()
{
//  uint8_t status = BMA400.getStatus();  
//  if(status & 0x01) Serial.print("wakeup interrupt!");
//  if(status & 0x04) Serial.print("gen1   interrupt!");
  ...
  if(BMA400_sleep_flag)
  {
   ...
   detachInterrupt(BMA400_intPin1);      // Detach the BMA400 "Go to sleep" interrupt so it doesn't spuriously wake the STM32L0
