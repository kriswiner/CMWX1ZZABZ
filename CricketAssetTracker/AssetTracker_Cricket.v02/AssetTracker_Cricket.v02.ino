...
uint16_t Hour = 0, Minute = 0, Second = 0, Millisec, Year = 1, Month = 1, Day = 1;
uint16_t hour = 0, minute = 0, second = 0, year = 1, month = 1, day = 1, millisec;
...
//  attachInterrupt(pps, CAMM8QintHandler, RISING);
   ...
   */
    ...
    LoRaTimer.start(callbackLoRaTx, 10000, 60000);      //  10 minute period, delayed 5 minutes
...
void loop()
{
  ...
  if (myLocation.fixType() != GNSSLocation::TYPE_NONE)
  {
      ...
      Serial.println();
        ...
        // Put the CAM M8Q to sleep once 3D fix with ufficient accuracy is obtained
